...
@app.route('/posts')
def posts():
    all_posts = MyBlog.query.order_by(MyBlog.posted_on).all()
    return render_template('posts.html', posts=all_posts)

@app.route('/posts/delete/<int:id>')
def delete(id):
    to_delete = MyBlog.query.get_or_404(id)
...
def edit(id):
    to_edit = MyBlog.query.get_or_404(id)
...
def new_post():
    if request.method == 'POST':
        ...
        new_post = MyBlog(title=post_title,
